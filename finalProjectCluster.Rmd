...
Making a dataframe that has only the fraud and ok transactions. We will use this data frame to cluster, find clusters that contain only fraud or ok transactions.
...
scaledUnkown = scale(unkDataFrame[,c(3,4,6)])

for(i in 1:nUnkn) {
  delta1 = c(as.numeric(scaledUnkown[i,])-as.numeric(clusterCenterSave[1,]))
  delta1 = sqrt(sum(delta1^2))
  delta2 = c(as.numeric(scaledUnkown[i,])-as.numeric(clusterCenterSave[2,]))
...
This provides the 50 fradulent trnsactions that need to be investigated.
...
```{r}
hist(saveTrans[sortSaveTrans$ix[1:50],2], main = 'dist to fraud', xlab = 'distr',freq = FALSE)
```
